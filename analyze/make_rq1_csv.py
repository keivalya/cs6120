#!/usr/bin/env python
"""analyze/make_rq1_csv.py — write report/rq1_causal.csv from raw logs (§7,§11).

Combines analyze/tsr.py (TSR/ΔTSR) and analyze/css.py (CSS, Follow/Ignore/Fail,
...
def main():
    ...
    (REPO_ROOT / "report" / "rq1_causal_caption.txt").write_text(
        ...
        f"See report/scene_fixed_check.json for the fixed-scene proof (§5/§12).\n"
    )
    print(f"wrote {out}")
    print(open(out).read())
