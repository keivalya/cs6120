#!/usr/bin/env python3
"""core/analyze/attention_extract.py — REAL layer-wise noun-vs-verb attention (RQ4).

This replaces the fabricated core/analyze/attention_mechanics.py (removed; see
paper/KNOWN_ISSUES.txt). That script emitted a closed-form curve
...
Output: ONE SHARD PER MODEL, paper/rq4_attention_<model>.csv, with columns
    model, layer, noun_attention, verb_attention, AAR_noun_over_verb, n_instructions
then `--merge` concatenates the shards into paper/rq4_attention.csv.
...
-----------------------------------------------
The first RQ4 run shipped a combined paper/rq4_attention.csv whose openvla_oft
...
observation, as the DESIGN above and paper/RQ4_RQ5_SPEC.txt require.
"""
...
def load_instructions(suite: str, max_tasks: int | None):
    """Read the original (uncorrupted) instruction per task from core/perturb/generated."""
    gen = REPO_ROOT / "data" / "instructions" / f"{suite}.jsonl"
    if not gen.exists():
        _die(f"missing {gen}; run core/perturb/make_instructions.py first")
    out = []
    seen_tasks = set()
...
class OpenVLAAttentionExtractor:
    ...
    NUM_WAIT = 10  # settle steps, same as core/run/eval_task_openvla.py
    ...
    def _real_observation(self, task_id: int, suite: str = "libero_goal"):
        ...
        Mirrors core/run/eval_task_openvla.py end to end: get_libero_image (:156-160,
...
def shard_path(model_alias: str) -> Path:
    return REPO_ROOT / "paper" / f"rq4_attention_{model_alias}.csv"
...
def log_to_manifest(model_alias: str, suite: str, results: list[dict], out_path: Path) -> None:
    ...
    append_manifest(REPO_ROOT / "MANIFEST.json", {
        ...
        "runner": "core/analyze/attention_extract.py",
...
def main():
    ...
    ap.add_argument("--merge", action="store_true",
                    help="combine the per-model shards into paper/rq4_attention.csv")
    ap.add_argument("--stdout", action="store_true", help="Print CSV content directly to stdout")
    args = ap.parse_args()

    combined = REPO_ROOT / "paper" / "rq4_attention.csv"
