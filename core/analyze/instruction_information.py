#!/usr/bin/env python
"""core/analyze/instruction_information.py — what does the instruction actually carry?
...
core/perturb/make_instructions.py: removing the verb should cost ~nothing, masking the
nouns should be catastrophic. If verb_dropped collapses, this account is wrong.

Usage:  python core/analyze/instruction_information.py [--results_root results]
Writes: paper/instruction_information.csv
"""
...
def main():
    ...
    if not any_run:
        print("  not yet run — see core/perturb/generated/libero_goal.jsonl "
