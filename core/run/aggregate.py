#!/usr/bin/env python
"""core/run/aggregate.py — combine per-task eval_task outputs into the standard
...
per task). This aggregates them, per (model, suite, condition, seed) dir, into:
  - episodes.jsonl  (concatenation of task*.jsonl, so core/analyze/* work unchanged)
...
paper/scene_fixed_check_<model>_<suite>.json, and a FAILING check exits 2 — the
causal claims are only valid while it passes.

Usage: python core/run/aggregate.py [--model smolvla --suite libero_goal --results_root ...]
"""
...
def main():
    ...
    atomic_write_json(REPO_ROOT / "paper" / f"scene_fixed_check_{args.model}_{args.suite}.json", check)
