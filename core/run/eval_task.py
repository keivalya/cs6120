#!/usr/bin/env python
"""core/run/eval_task.py — one LIBERO task, all conditions, SmolVLA (CLAUDE.md §5,§7).
...
task, so per-task processes never clash). core/analyze/* aggregate across task*.jsonl.

Usage:
  python core/run/eval_task.py --task_id 1 --conditions original,blank,nonsense \
    --seed 7 --n_episodes 2
"""
...
# RQ1.2 (§7): parse ALL Goal task goals once for achieved_task_ids (core/analyze/goal_eval.py).
...
# is confounded (see core/run/scene_atomic.py for the measured evidence). Paraphrase mode
...
for label, items in groups:
    ...
    # double-counted by PRIDE (see core/analyze/dedupe_results.py). Every record
