#!/usr/bin/env python
"""core/run/eval_task_oft.py — one LIBERO task, all conditions, OpenVLA-OFT 7.5B (RQ1.3).
...
executed open-loop. So instead of inlining (as core/run/eval_task_openvla.py does for
...
prismatic/__init__.py; see paper/GATE4_STATUS.md / paper/patches/).

Same fixed-scene protocol + OUTPUT CONTRACT as core/run/eval_task{,_openvla}.py:
...
so core/run/aggregate.py + core/analyze/* work unchanged.
...
# RQ1.2 (§7): parse ALL Goal task goals once for achieved_task_ids (see core/analyze/goal_eval.py).
...
# the task's paraphrases appended to one file (core/run/para_util.py).
if args.paraphrase_axis:
    ...
    # See the same block in core/run/eval_task.py. THIS runner is where the failure was
...
for label, items in groups:
    ...
    # Resume, don't restart — see the same block in core/run/eval_task_openvla.py.
