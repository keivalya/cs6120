#!/usr/bin/env python
"""core/run/eval_task_openvla.py — one LIBERO task, all conditions, OpenVLA-7B (RQ1.3).

Mirrors core/run/eval_task.py (SmolVLA) exactly in OUTPUT CONTRACT and the reliable
...
SmolVLA analysis, so core/analyze/* work unchanged.

Output: results/openvla/<suite>/<condition>/seed<k>/task<tid>.jsonl  (same fields
as eval_task.py). Usage mirrors eval_task.py:
  python core/run/eval_task_openvla.py --task_id 7 --conditions original,blank,wrong_task \
...
# openvla_utils.py}. See paper/patches/ note in GATE4_STATUS.md.
# --------------------------------------------------------------------------- #
...
# state satisfies (achieved_task_ids) -> exact Follow/Ignore/Fail in core/analyze/css.py.
...
# whose items are the task's paraphrases (all appended to one file). See core/run/para_util.py.
if args.paraphrase_axis:
    ...
    # See the same block in core/run/eval_task.py: a requested condition with no record
    # is a hard error. Job 8853538 built one group from a 7-condition request and
    # exited 0 having run only `original`.
    ...
    if by_design:
        print(f"[ov {tid}] skip by design (no valid swap): {','.join(by_design)}", flush=True)
    groups = [(c, [(true_lang if c == "original" else instr[c], {})])
              for c in conditions if (c == "original" or c in instr)]
    print(f"[ov {tid}] grid: {len(groups)} conditions x {args.n_episodes} eps "
...
# contrast is confounded. This model's whole grid was broken that way — its
# conditions were split across separate jobs. See core/run/scene_atomic.py.
...
for label, items in groups:
    ...
    # Resume, don't restart: this used to open "w", so an 8h wall-time kill in the
    # middle of a task threw away every episode already rolled out for it and the
    # resubmit started from episode 0. Top up in append mode instead, exactly as
    # core/run/eval_task.py does. Episode k keeps its init state either way, so the
    # scene-fixed matching survives a resume.
    existing_records = []
    if out_file.exists():
        try:
            with open(out_file) as f:
