"""core/run/para_util.py — load per-task paraphrase items for RQ2 eval (shared by runners).

Reads core/perturb/generated/<suite>_paraphrases.jsonl (produced by
core/perturb/make_paraphrases.py) and returns the paraphrases for one (task, axis),
...
def load_task_paraphrases(suite, task_id, axis, max_per_axis=None, seed=0):
    ...
    path = REPO_ROOT / "data" / "instructions" / f"{suite}_paraphrases.jsonl"
    rows = []
