#!/bin/bash
# core/run/rebuild_envs.sh — rebuild the vla-* conda envs from the pinned pip locks.
#
...
#
# Usage:
#   bash core/run/rebuild_envs.sh                 # rebuild all three
#   bash core/run/rebuild_envs.sh vla-smolvla     # rebuild just one
#   CONDA_ENVS=/work/pandya.kei/vla-envs bash core/run/rebuild_envs.sh   # persistent location
#
...
echo "  sbatch --export=ALL,CONDA_ENVS=$CONDA_ENVS,SEED_ONLY=7 core/run/slurm/p2_rq1_smolvla.sbatch"
