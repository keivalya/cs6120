#!/bin/bash
# core/run/rebuild_oft_minimal.sh — rebuild the vla-oft env (OpenVLA-OFT-7.5B RQ1.3).
#
# See core/run/rebuild_openvla_minimal.sh for why we don't replay envs/vla-oft.lock.txt.
#
# WHAT MAKES THIS ENV DIFFERENT (and why the step ORDER below is load-bearing):
# core/run/eval_task_oft.py reuses the OFT repo's experiments.robot.* helpers, so
...
#   sbatch core/run/slurm/rebuild_7b_minimal.sbatch
set -uo pipefail
...
if ! grep -q "PATCHED" "$REPO/openvla-oft/prismatic/__init__.py" 2>/dev/null; then
  echo "[patch] re-applying paper/patches/oft_prismatic_init_lazy.diff"
  patch -p0 -d "$REPO" < "$REPO/paper/patches/oft_prismatic_init_lazy.diff" || {
...
echo "Validate on a GPU node:  $PREFIX/bin/python core/run/smoke_oft.py"
