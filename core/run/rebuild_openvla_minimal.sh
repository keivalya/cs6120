#!/bin/bash
# core/run/rebuild_openvla_minimal.sh — rebuild the vla-openvla env (OpenVLA-7B RQ1.3).
#
...
# is what core/run/eval_task_openvla.py:115-125 exists to route around). So: install
...
# pin from core/run/rebuild_oft_minimal.sh — that downgrade exists only because OFT
...
# core/run/smoke_openvla.py on a GPU node). conda create OOMs on the login node, so run
# it in an allocation:  sbatch core/run/slurm/rebuild_7b_minimal.sbatch
set -uo pipefail
...
echo "Validate on a GPU node:  $PREFIX/bin/python core/run/smoke_openvla.py"
