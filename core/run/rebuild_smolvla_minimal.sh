#!/bin/bash
# core/run/rebuild_smolvla_minimal.sh — minimal fresh vla-smolvla env (top-level deps).
#
...
# test that imports exactly what core/run/eval_task.py imports.
#
# MUST run inside an allocation (login node OOM-kills conda create). Use the
# wrapper: sbatch core/run/slurm/rebuild_smolvla_minimal.sbatch
set -uo pipefail
...
echo "[smoke] importing exactly what core/run/eval_task.py needs..."
