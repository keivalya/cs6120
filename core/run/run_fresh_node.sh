#!/bin/bash
# core/run/run_fresh_node.sh — turnkey RQ2 + RQ1-gap sweep for a FRESH GPU allocation.
#
...
# Usage:  bash core/run/run_fresh_node.sh [A|B|ALL]   (default ALL)
...
if [ "$PHASE" = "A" ] || [ "$PHASE" = "ALL" ]; then
  ...
  for s in $SEEDS_CORE; do
    ...
    runner "$OV"  core/run/eval_task_openvla.py "$OVENV" --conditions wrong_object,wrong_action,wrong_task --seed $s --n_episodes 2
    runner "$OFT" core/run/eval_task_oft.py     "$OVENV" --conditions wrong_object,wrong_action,wrong_task,repeated --seed $s --n_episodes 2
    # smolvla: seed 42 non-wrong (seed 7 already present) + wrong_* both seeds (goal-logged)
    runner "$SM"  core/run/eval_task.py         "" --conditions original,blank,nonsense,repeated,wrong_object,wrong_action,wrong_task --seed $s --n_episodes 2 --obs_hw 256
    # oft: seed 42 core causal (seed 7 present) so mean±std
    [ "$s" = "42" ] && runner "$OFT" core/run/eval_task_oft.py "$OVENV" --conditions original,blank,nonsense --seed $s --n_episodes 2
  done
fi
...
if [ "$PHASE" = "B" ] || [ "$PHASE" = "ALL" ]; then
  ...
  for ax in para_object para_action para_compositional; do
    ...
    runner "$SM"  core/run/eval_task.py         "" --paraphrase_axis $ax --seed 7 --n_episodes 1 --obs_hw 256
    # 7B: stratified sample (~15/task/axis), 1 episode/paraphrase
    runner "$OV"  core/run/eval_task_openvla.py "$OVENV" --paraphrase_axis $ax --max_per_axis 15 --seed 7 --n_episodes 1
    runner "$OFT" core/run/eval_task_oft.py     "$OVENV" --paraphrase_axis $ax --max_per_axis 15 --seed 7 --n_episodes 1
  done
fi

echo "### AGGREGATE + ANALYZE ###" | tee -a "$LOG"
for m in smolvla openvla openvla_oft; do
  $OV core/run/aggregate.py --model $m --suite libero_goal >> "$LOG" 2>&1
  $OV core/analyze/make_rq1_csv.py --model $m --suite libero_goal >> "$LOG" 2>&1
done
$OV core/analyze/make_rq1_scale.py >> "$LOG" 2>&1
$OV core/analyze/plot_rq1.py       >> "$LOG" 2>&1
$OV core/analyze/make_rq2_csv.py --suite libero_goal >> "$LOG" 2>&1
$OV core/analyze/plot_rq2.py       >> "$LOG" 2>&1
echo "=== DONE $(date) — see paper/rq1_*.csv, rq2_*.csv, *.png ===" | tee -a "$LOG"
