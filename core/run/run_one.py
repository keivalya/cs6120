#!/usr/bin/env python
"""core/run/run_one.py — single (model, suite, condition, seed) eval -> logs.
...
NOTE: for SmolVLA the RELIABLE entry point is core/run/eval_task.py (ONE process per
task = one EGL context), driven by core/run/launch.py and aggregated by
core/run/aggregate.py. This run_one.py builds many env contexts in one process, which
...
def extract_object_poses(raw_obs: dict) -> dict:
    ...
    core/analyze/css.py Follow/Ignore/Fail and core/analyze/locus.py)."""
    out = {}
...
def load_instructions(suite: str, condition: str) -> dict[int, str] | None:
    ...
    core/perturb/generated/<suite>.jsonl (produced by make_instructions.py, GATE 3).
    ...
    if not gen.exists():
        raise FileNotFoundError(
            ...
            f"core/perturb/make_instructions.py first (expected {gen})."
        )
...
def main():
    ...
    ap.add_argument("--results_root", type=str, default=str(REPO_ROOT / "data" / "results"))
    args = ap.parse_args()
