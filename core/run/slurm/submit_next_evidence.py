...
def main():
    ...
    for i, (tid, cond, axis) in enumerate(CASES):
        ...
        # Submit it!
        if axis:
            cmd = f"sbatch --array={tid}-{tid} core/run/slurm/run_video_array.sbatch --paraphrase_axis {axis}"
        else:
            cmd = f"sbatch --array={tid}-{tid} core/run/slurm/run_video_array.sbatch --condition {cond}"
