#!/usr/bin/env python
"""core/run/smoke_oft.py — prove the vla-oft env can actually produce an action chunk.

Mirrors core/run/eval_task_oft.py: it reuses the OFT repo's experiments.robot.* helpers,
...
wandb pin in core/run/rebuild_oft_minimal.sh exists to make them succeed, and if one has
drifted this fails here in seconds instead of three hours into a production job.

Run on a GPU node with the vla-oft python:
    $E/vla-oft/bin/python core/run/smoke_oft.py
"""
...
# same way core/run/eval_task_oft.py:118 does it. This insert must therefore come BEFORE
