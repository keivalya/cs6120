...
\subsection{Vision-Language-Action Models \& Checkpoints}
VLA models extend vision-language models with action heads to output motor controls conditioned on language instructions \citep{brohan2022rt1,brohan2023rt2,driess2023palm,open_x_embodiment_rt_x_2023}. We evaluate three open-source checkpoints spanning a $16\times$ parameter range and three distinct action parameterizations: (1) \textbf{SmolVLA-450M} \citep{shukor2025smolvla}, a compact policy combining SmolVLM2 with a flow-matching action expert; (2) \textbf{OpenVLA-7B} \citep{kim2025openvla}, emitting discretized action tokens from a static camera; and (3) \textbf{OpenVLA-OFT-7.5B} \citep{kim2025oft}, utilizing continuous action chunking via L1 regression, an additional wrist camera, and 8D proprioception.
...
Using $\mathcal{A}(S_T)$, we classify episode outcomes under perturbed prompts:
\begin{itemize}
    \item \textbf{Destructive Conditions (\texttt{blank}, \texttt{nonsense}):} An episode is \emph{Grounded} if $I_{\text{orig}}$ succeeds but $\mathcal{A}(S_T) = \emptyset$ (removing language eliminates success). It is categorized as \emph{Memorization} if $\mathcal{A}(S_T) \neq \emptyset$ despite missing language.
    \item \textbf{Misleading Conditions (\texttt{wrong\_*})}: When $I_{\text{wrong}}$ specifies goal $k_{\text{wrong}} \neq k^*$, we measure whether the policy complies with the new goal (\emph{Follow}, $k_{\text{wrong}} \in \mathcal{A}(S_T)$), defaults to the scene's original goal (\emph{Ignore}, $k^* \in \mathcal{A}(S_T)$), or fails both (\emph{Fail}).
\end{itemize}

\subsection{Metrics}
\label{subsec:metrics}
We convert episode classifications into four aggregate metrics: (1) \textbf{Causal Sensitivity Score (CSS):} For destructive conditions, $\text{CSS}(c) = \frac{\text{TSR}_0 - \text{TSR}(c)}{\max(\text{TSR}_0, \epsilon)}$, where $\text{CSS} \approx 1$ indicates strong causal necessity; (2) \textbf{Original-Action Rate (OAR):} For misleading conditions, $\text{OAR}(c) = \frac{|\text{Ignore}|}{|\text{Follow}| + |\text{Ignore}| + |\text{Fail}|}$, measuring scene-memorization overrides; (3) \textbf{PRIDE Score:} For paraphrases, task success weighted by paraphrase distance \citep{kim2026liberopara}; and (4) \textbf{Failure Locus:} Categorizing failures into \emph{planning} vs. \emph{execution} breakdowns.
...
Table~\ref{tab:rq1_causal} presents Task Success Rate (TSR) and Causal Sensitivity Score (CSS) across all evaluated model architectures under canonical (\texttt{original}), destructive (\texttt{blank}, \texttt{nonsense}), misleading (\texttt{wrong\_action}, \texttt{wrong\_object}, \texttt{wrong\_task}), and control (\texttt{repeated}) prompt conditions (column definitions detailed in Table~\ref{tab:rq1_causal} caption).
