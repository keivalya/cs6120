...
\caption{\textbf{Word-Class Removal Impact on OpenVLA-OFT-7.5B.} Task Success Rate drop (percentage points) under complete verb deletion ($1.1$ words removed, $0$ bits destroyed) versus noun masking ($2.4$ words removed, $3.32$ bits destroyed) compared to destructive blank prompt ($7.2$ words removed). Deleting the verb causes an insignificant $2.0$ pp drop ($p=0.34$), whereas masking nouns collapses performance by $73.5$ pp.}
\label{fig:removal}
\end{figure}
