#!/bin/bash
# run/make_submission_zip.sh — build a self-contained LaTeX source bundle.
#
...
#
#   bash run/make_submission_zip.sh
...
cp report/paper_acl.tex report/references.bib report/acl.sty report/acl_natbib.bst "$STAGE"/
cp report/tables/*.tex "$STAGE"/tables/

# Only the figures the paper actually includes -- report/ holds stale ones too
# (e.g. rq4_attention.png, from the withdrawn attention analysis).
for f in $(grep -o '\\includegraphics\[[^]]*\]{[^}]*}' report/paper_acl.tex \
           | sed 's/.*{//;s/}//'); do
  if [ -f "report/$f" ]; then cp "report/$f" "$STAGE"/; else echo "MISSING FIGURE: $f"; exit 1; fi
done

# Validate the staged copy, not the working tree: this is what actually ships.
cp report/check_tex.py "$STAGE"/
