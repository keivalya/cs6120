#!/usr/bin/env python
"""run/preflight_libero.py — model-free render/GPU gate that works in ANY of the envs.

run/preflight.py loads SmolVLA, so it can only ever validate the vla-smolvla env.
The 7B jobs were gating themselves on it anyway (run/slurm/p2_rq1_{openvla,oft}.sbatch
...
Run it with the env's OWN python:
    $E/vla-openvla/bin/python run/preflight_libero.py
    $E/vla-oft/bin/python     run/preflight_libero.py

Keep run/preflight.py for the SmolVLA job — it is the only check that reproduces
