...
def main():
    ...
    for tid, cond, axis in CASES:
        ...
        if axis:
            cmd = f"sbatch {dep_arg} --array={tid}-{tid} run/slurm/run_video_array.sbatch --paraphrase_axis {axis}"
        else:
            cmd = f"sbatch {dep_arg} --array={tid}-{tid} run/slurm/run_video_array.sbatch --condition {cond}"
