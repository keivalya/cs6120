#!/usr/bin/env python
"""run/smoke_openvla.py — prove the vla-openvla env can actually do a rollout step.

Mirrors run/eval_task_openvla.py's import order and does real work: bf16 sdpa load
...
Run on a GPU node with the vla-openvla python:
    $E/vla-openvla/bin/python run/smoke_openvla.py
"""
...
# The design decision at run/eval_task_openvla.py:115-125 is that prismatic must
...
# The two tf ops run/eval_task_openvla.py:148-153 calls on every frame.
